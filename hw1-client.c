...
void mealy(int msock)
{
    ...
    // make another char variable to store what client receive from server.
    char buffrecv[MAXDATASIZE] = "";
    int numbytes;
    // ask user if they want to make a call, y to continue, n to end this program.
    printf("Do you want to make a call? Type y to call, n to cancel\n");
    if(getchar()=='y')
    {
        // keep sending RING until receive something from server.
        strcpy(buf,"RING");
        do{
            mealysend(msock,buf,strlen(buf));
        }
        while (mealyrecvto(msock, buffrecv, MAXDATASIZE, 5)==-1);
        strcpy(buf,"HELLO");// send HELLO and switch state.
        mealysend(msock,buf,strlen(buf));
        printf("Now you are in the Talk state.\n");
        printf("Type in your data, type n to cancel.\n");
        char data[MAXDATASIZE];// make a variable to store information from user.
        //keep let user input, until they input n.
        while(scanf("%s", &data)==1)
        {
            ...
            //and symbol DATA before input string.
            strcpy(buf,"DATA ");
            strcat(buf, data);
            printf("User input: %s\n", data);
            mealysend(msock,buf,strlen(buf));
            printf("Any more data? type n to cancel.\n");
        }
        //ending three way handshake.
        strcpy(buf,"FINISHED");
        printf("Now you are in the Ending state\n");
        mealysend(msock,buf,strlen(buf));
        numbytes = mealyrecvto(msock, buffrecv, MAXDATASIZE, 5);
        strcpy(buf,"BYE");
        mealysend(msock,buf,strlen(buf));
        printf("Now you return to Idle state\n");
    }
    else if(getchar()=='n')
    {
        printf("No call is required, back to idle.\n");
    }
